...
def rungame(gameid, awayteam, hometeam, is_neutral, is_nba, filePath):
    ...
    nba_match = GetNeutralValue(is_nba)
    ...
    t1StealBlock = round(sum(cp['Stealing'] for cp in t1rosterRaw_df))
    t1Ballwork = round(sum(cp['Ballwork'] for cp in t1rosterRaw_df))    
    t2Rebounding = round(sum(cp['Rebounding'] for cp in t2rosterRaw_df))
    t2StealBlock = round(sum(cp['Stealing'] for cp in t2rosterRaw_df))
    t2Ballwork = round(sum(cp['Ballwork'] for cp in t2rosterRaw_df))
    t1InsPro = sum(t1rosterRaw_df['InteriorProportion'])
    t1MidPro = sum(t1rosterRaw_df['MidRangeProportion'])
    t13ptPro = sum(t1rosterRaw_df['ThreePointProportion'])
    t2InsPro = sum(t2rosterRaw_df['InteriorProportion'])
    t2MidPro = sum(t2rosterRaw_df['MidRangeProportion'])
    t23ptPro = sum(t2rosterRaw_df['ThreePointProportion'])
    
    t1roster_df = team_one_roster
    
    for x in t1rosterRaw_df:
        cp = Player(x, t1InsShootingBonus, t1MidShootingBonus, t13ptShootingBonus, t1BallworkBonus, t1ReboundingBonus)
        cp.get_advanced_stats(t1Rebounding, t1StealBlock, t1Ballwork, t1InsPro, t1MidPro, t13ptPro, t1TurnoverBonus)
        team_one_roster.append(cp)
    

    for x in t2rosterRaw_df:
        cp = Player(x, t2InsShootingBonus, t2MidShootingBonus, t23ptShootingBonus, t2BallworkBonus, t2ReboundingBonus)
        cp.get_advanced_stats(t2Rebounding, t2StealBlock, t2Ballwork, t2InsPro, t2MidPro, t23ptPro, t2TurnoverBonus)
    ...
    t1InsPro = sum(cp['InteriorProportion'] for cp in t1roster_df)
    ...
    t2InsPro = sum(cp['InteriorProportion'] for cp in t2roster_df)
    ...
    t2threeptAttemptGPAdj = round((0.81*(t23ptPro/100))-threeptAttemptProbability,5)
    ...
    while possessionNum <= total_possessions:
        if possessionNum == 0:
            ...
            if tipOff < t1TipChance:
                possession = t2
                collector.AppendPlay(t2, t2Tip2.FirstName + " wins the tipoff for " + t2 + "!", "Tipoff", 0, 0, 0, total_possessions)
            else:
                possession = t1
                collector.AppendPlay(t1, t1Tip2.FirstName + " wins the tipoff for " + t1 + "!", "Tipoff", 0, 0, 0, total_possessions)
        ...
        if possession == t1:
            ...
            if possrand < t1StealCutoff:
                ...
                pickPlayer = random.choices(gett1Players,weights=[x.ThreePointUsage for x in team_one_roster],k=1)
                ...
                if eventOutcome < made3Cutoff:
                    ...
                    shooter.Stats.AddFieldGoal(True, 3)
                    team_one.Stats.AddThreePointShot(True)
                    team_one.Stats.AddPoints(3, possessionNum, total_possessions / 2, is_ot)
                    team_one.Stats.CalculateLead(3, t1pts - t2pts)
                    assistRand = random.random()
                    if assistRand > 0.173:
                        assistList = [x for x in gett1Players if x.ID != shooter.ID]
                        pickAssister = random.choices(assistList,weights=t1roster_df['assistper'],k=1)
                ...
                pickPlayer = random.choices(gett1Players,weights=[x.MidUsage for x in team_one_roster],k=1)
                shooter = pickPlayer[0]
                shooter.Stats.AddPossession()
                printShooter = shooter.FirstName + " " + shooter.LastName
                ...
                pickPlayer = random.choices(gett1Players,weights=[x.InsideUsage for x in team_one_roster],k=1)
                shooter = pickPlayer[0]
                shooter.Stats.AddPossession()
                printShooter = shooter.FirstName + " " + shooter.LastName
                blockAdj = (0.00001*t1roster_df['interiordefrate'].sum())-0.0153
                if shooter.ID == t2team_df['FocusPlayer']:
            ...
            if possrand < t2StealCutoff:
                ...
                pickPlayer = random.choices(gett2Players,weights=[x.ThreePointUsage for x in team_two_roster],k=1)
                shooter = pickPlayer[0]
                shooter.Stats.AddPossession()
                ...
                pickPlayer = random.choices(gett2Players,weights=[x.MidUsage for x in team_two_roster],k=1)
                shooter = pickPlayer[0]
                shooter.Stats.AddPossession()
                printShooter = shooter.FirstName + " " + shooter.LastName
                ...
                pickPlayer = random.choices(gett2Players,weights=[x.InsideUsage for x in team_two_roster],k=1)
                shooter = pickPlayer[0]
                printShooter = shooter.FirstName + " " + shooter.LastName
                blockAdj = (0.00001*t2_adj_interior_defense)-0.0153
                if shooter.ID == t1team_df['FocusPlayer']:
        ...
        if possessionNum == math.floor((total_possessions)/4) and nba_match:
            ...
            print(t1 + ": " + str(t1pts))
            msg = ""
            collector.AppendPlay("", msg, "END OF 1ST QUARTER", t1pts, t2pts, possessionNum, total_possessions)
            print("\n")
        ...
        if possessionNum == math.floor((total_possessions)/1.5) and nba_match:
            ...
            print(t1 + ": " + str(t1pts))
            msg = ""
            collector.AppendPlay("", msg, "END OF 3rd QUARTER", t1pts, t2pts, possessionNum, total_possessions)
            print("\n")
